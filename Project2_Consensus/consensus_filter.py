...
class Node:
	...
	def max_degree(self):
		...
		for neighbor_name in self.neighbor_readings:
			...
			if self.name == "0" and self.count == 0:

				print "Acc ", acc_reading, " n_v ", neighbor_value, " w ", weight

		if self.name == "0" and self.count == 0:
			# Set the weight of this node so that the weights all sum to 1
			print "Self deg ", self.get_degree(), " tot nodes ", self.network.total_nodes(), " div ",  self.get_degree() / float(self.network.total_nodes())
			print "self val ", self.get_sensor_reading()
	...
	def __init__(self, environment, network, name, reading_noise=.01, range_s=1.6, consensus_method = "Max Degree"):
		...
		self.stable_reading = np.random.normal(scale=500.0, size=(T_space,)) + self.environment.get_target_reading_at(0)
	...
	def get_sensor_reading(self):
		...
		if self.name == "0" and self.count == 0:
			...
			print "position ", self.get_position(), " avg_pos ", self.network.get_network_average_position()
			print "noise ", noise
...
class Simulate:
	...
	def run(self, iterations = 1000):
		...
		for time_step in range(iterations):
			...
			# Log the info
			#self.info[time_step] = self.environment.get_target_position(), avg, std
			self.info.append([self.environment.get_target_position(), avg, std])
	...
	def visualize(self):
		...
		for item in self.info:
			...
			target_prediction.append(item[1])

		plt.ylim([min(target_prediction) - 20, max(target_prediction) + 20])

		plt.plot(target_prediction, 'b-', label="Network Average Prediction")
