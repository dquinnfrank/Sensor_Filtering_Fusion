...
class Node:
	...
	# Sets the reading for this node, and returns it
	...
	def reading(self):
		...
		#self.stable_reading = self.fuse_readings()
...
class Environment:
	...
	#
	# min_neighbors : int
	# The minimum of neighbors that any node can have
	# This is to make sure that the graph is always connected
	# Defaults to 2
	def __init__(self, target_function, communication_radius = None, max_neighbors = None, min_neighbors = 2):
	...
	def get_node_neighbors(self, node_name):
		...
		for name in self.nodes:
			...
			if node_name != name:
				...
				elif distance <= communication_radius:
