...
class Simulate:
	...
	#
	# default_consensus_method : string
	def __init__(self, max_range, num_nodes = 10, default_consensus_method = "Max Degree"):
		...
		self.environment = Environment(lambda t : np.array([max_range / 2.0, max_range / 2.0]), lambda t : np.array([50.0]), communication_radius = 1.7)
	...
	def visualize(self):
		...
		# Get the target signal at all time points
		target_signal = []
		for time_step in range(len(self.info)):

			target_signal.append(self.environment.get_target_reading_at(time_step))
		...
		plt.ylim([min(target_prediction) - 1, max(target_prediction) + 1])
		...
		print "Average of last 5 network predictions: ", np.average(target_prediction[-5:], axis = 0)
...
if __name__ == "__main__":
	...
	sim = Simulate(3, default_consensus_method = "Weight Design 1")
